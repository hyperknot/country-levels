...
def get_state_data():
    ...
    population_by_int = get_population_by_state()
    ...
    for geocode_row in all_geocode_data:
        ...
        population = population_by_int.get(state_int_code)
...
def get_county_data():
    all_geocode_data = get_all_geocode_data()
    population_data = get_population_by_county()
...
def get_population_by_state():
    ...
    with open(fips_data_dir / 'population.csv', newline='', errors='surrogateescape') as csvfile:
        ...
        for row in reader:
            ...
            state_code_int = int(row['STATE'])
            population = int(row['POPESTIMATE2019'])
            states_by_int[state_code_int] = population

    return states_by_int


def get_population_by_county():
    counties_by_str = {}
